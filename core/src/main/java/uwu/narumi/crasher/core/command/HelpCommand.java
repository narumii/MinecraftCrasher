...
public class HelpCommand extends Command {
  ...
  public void execute(String... args) throws CommandException {
    if (args.length > 0) {
      Command command = Crasher.INSTANCE.getCommandManager().getCommand(args[0]).orElseThrow(()-> new CommandException(""));
      System.out.printf("%s: %s - (%s)%n", command.getAlias(), command.getDescription(), command.getUsage());
      return;
    }
    ...
    Crasher.INSTANCE.getCommandManager().getCommands().stream()
        ...
        .forEach(command ->
            System.out.println(String.format("%s: %s - (%s)", command.getAlias(), command.getDescription(), command.getUsage())));
  }
}
